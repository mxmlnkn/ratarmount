...
void
crc_init( unsigned int* crc_table,
          int           little_endian )
{
    unsigned int i;
...
static unsigned int
get_bits( struct bunzip_data* bd,
          char                bits_wanted )
{
    unsigned int bits = 0;
 ...
 */
static int
read_block_header( struct bunzip_data* bd,
                   struct bwdata*      bw )
{
 ...
 */
static int
read_huffman_data( struct bunzip_data* bd,
                   struct bwdata*      bw )
{
    struct group_data* hufGroup;
...
void
flush_bunzip_outbuf( struct bunzip_data* bd,
                     int                 out_fd )
{
    if ( bd->outbufPos ) {
...
void
burrows_wheeler_prep( struct bunzip_data* bd,
                      struct bwdata*      bw )
{
    int ii, jj;
    unsigned int* dbuf = bw->dbuf;
...
int
read_bunzip_data( struct bunzip_data* bd )
{
...
int
write_bunzip_data( struct bunzip_data* bd,
                   struct bwdata*      bw,
                   int                 out_fd,
                   char*               outbuf,
                   int                 len )
...
public:
    ...
    {
        const auto rc = write_bunzip_data( &bd, &bd.bwdata[0], outputFileDescriptor, outputBuffer, outputBufferSize );
        flush_bunzip_outbuf( &bd, outputFileDescriptor );
                        ...
                        const size_t size )
{
    ...
    //std::memcpy( bd.crc32Table, &crc32Table, sizeof( bd.crc32Table ) );
