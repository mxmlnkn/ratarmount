...
def plotSummary(logFiles):
    ...
    for ikey, key in enumerate(['tinsert', 'tselectpath', 'tselecthash', 'tselectpathstart']):
        ax = fig.add_subplot(
            2, 2, 1 + ikey, xlabel='Number of Rows', ylabel=key + ' per row / s', xscale='log', yscale='log'
        )
...
for nFiles in [1000 * 1000]:
    ...
    for iSchema, schema in enumerate(schemas):
        ...
        axi.plot(insertTimes, linestyle='', marker='.', label=f'{labels[iSchema]}, total time: {tTotalInsert:.3f}s')
        ...
        def benchmarkSelect(entity, sqlCommand, makeRow, ax):
            ...
            ax.plot(
                selectTimes, linestyle='', marker='.', label=f'labels[iSchema], total time: {tTotalSelectTime:.3f}s'
            )

        benchmarkSelect(
            'paths', 'SELECT hash FROM files WHERE path == (?)', lambda j: (os.urandom(fileNameLength // 2).hex(),), axs
        )
        ...
        benchmarkSelect(
            'hashes', 'SELECT path FROM files WHERE "hash" == (?);', lambda j: (np.random.randint(0, nFiles),), axsh
        )
