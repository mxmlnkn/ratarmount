...
def compute_bandwidths(t):
    return {chunk_size: compute_statistics(file_size / np.array(times)) for chunk_size, times in t.items()}


results = {label: compute_bandwidths(times_per_chunk_size) for label, times_per_chunk_size in data.items()}
...
for label, stats_per_chunk_size in results.items():
    ...
    ax.bar(
        bar_positions - width / 2 + i_bar * width,
        times_mean,
        yerr=times_std,
        width=width,
        label=labels[label],
