...
class TestRemovePrefixMountSource:
    @staticmethod
    @pytest.mark.parametrize('path', ["foo", "/foo", "/folder/../foo", "folder/foo", "/folder/./foo", "/a/b/c/foo"])
    def test_single_file(path: str):
        contents = b"bar"
        ms = RemovePrefixMountSource(path, SingleFileMountSource("file", io.BytesIO(contents)))
